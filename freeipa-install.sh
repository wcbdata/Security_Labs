...
#domain needs to include subdomains in fqdn (e.g., if fqdn is somehost.foo.bar.spiffy.com, then domain should be foo.bar.spiffy.com)
export NAME=ipa
export DOMAIN=c.se-demos-wcb.internal
...
#if the previous step fails, may need to manully rerun these two steps:
